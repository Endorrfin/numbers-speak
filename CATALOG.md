...
| 6 | `crime-index` | Crime by country: homicides and perceived crime / Злочинність у країнах: убивства й відчуття небезпеки | `security` | ranked-bar | `RankedBar` | `_examples/Contribution/Demographics/Crime index` (data replaced) | EN + UA | own | P3a (S3‑rb) | **soon** (S3‑rb) | owner: two sub‑tabs (`?show=homicide|numbeo`) — UNODC homicide rate (data file 12 Jul 2026; latest year ≥ 2015, 166 countries, older years marked; UK combined from 3 jurisdictions; Iraq = Central Iraq) **done**; Numbeo 2026 Mid‑Year waits for the owner's manual copy (terms forbid scraping) → then `published`; Haiti = Americas (Q1) |
