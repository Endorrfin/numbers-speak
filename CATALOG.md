...
| 2 | `gdp-ppp-per-capita` | GDP (PPP) per capita, 2023–2025 / ВВП (ПКС) на душу населення, 2023–2025 | `economy` | ranked-bar | `RankedBar` | `_examples/Contribution/Demographics/GDP (PPP) per capita 2023` (data refreshed) | EN + UA | own | P3a (S3‑rb) | **published** (S3‑rb) | WB API NY.GDP.PCAP.PP.CD, WDI 2026‑07‑13, one vintage for 2023 · 2024 · 2025 (`?year=`), 197/195/185 of 217 economies, no fill‑ins; “× world average” = value ÷ WB World (WLD) aggregate, derived (Q2); legacy typed “world share” and nominal column dropped |
...
| 4 | `population-by-country` | Population by country, 2025 / Населення країн, 2025 | `world` | ranked-bar | `RankedBar` + `Strip` (share by region) | `_examples/Contribution/Demographics/Population density` (data replaced) | EN + UA | own | P3a (S3‑rb) | **published** (S3‑rb) | UN WPP 2024 medium variant, 1 July 2025, 237 countries/areas (CC BY 3.0 IGO); sub‑tabs population · density (`?metric=density`) — density = population ÷ land area joined at runtime with `land-area` (Q3), 234 of 237 (Guernsey, Jersey, Kosovo without area); < 25 km² marked approximate; UA/RU marked * (Crimea counted in Ukraine) |
...
| 6 | `crime-index` | Crime by country: homicides and perceived crime / Злочинність у країнах: убивства й відчуття небезпеки | `security` | ranked-bar | `RankedBar` | `_examples/Contribution/Demographics/Crime index` (data replaced) | EN + UA | own | P3a (S3‑rb) | **published** (S3‑rb) | owner: two sub‑tabs (`?show=homicide|numbeo`) — UNODC homicide rate (data file 12 Jul 2026; latest year ≥ 2015, 166 countries, older years marked; UK combined from 3 jurisdictions; Iraq = Central Iraq) **done**; Numbeo 2026 Mid‑Year (148 countries; owner's manual copy — terms forbid scraping; ties keep Numbeo's order) **done**; Haiti = Americas (Q1) |
| 7 | `global-peace-index` | Global Peace Index 2026 / Глобальний індекс миру 2026 | `security` | ranked-bar | `RankedBar` | `_examples/Contribution/Demographics/GPI-162` (data replaced) | EN + UA | own | P3a (S3‑rb) | **published** (S3‑rb) | IEP GPI 2026 report (June 2026), 163 countries: ranking table (pp. 10–11) cross‑checked against the regional tables (which give the score change); ties "=70"; order switch (most / least peaceful first); the report's Honduras conflict (97 in the ranking table, 96 in its regional table and text) kept as printed + noted; owner: built without writing to IEP — terms §12.5 (maps: educational, non‑commercial, with acknowledgement); bars start at the scale floor 1 (`SCORE_MIN`, S3‑fx — page 1, 1.161–1.538, looked flat from 0) with an axis title “Score on a 1–5 scale · bars start at 1”; open: removed if IEP objects (§9.5) |
| 8 | `robotization` | Industrial robots per 10,000 workers, 2024 (top 15) / Роботизація виробництва, 2024 (топ‑15) | `economy` | ranked-bar | `RankedBar` | `_examples/Contribution/Robotization of production` (data replaced) | EN + UA | own | P3a (S3‑rb) | **published** (S3‑rb) | IFR World Robotics 2025 (data 2024), press release 8 Apr 2026 + its chart of 22 economies (terms §5(2): single figures with credit); top 15 + “× world average” (132); Belgium & Luxembourg one row (*); China 22nd stated from data |
