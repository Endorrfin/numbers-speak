...
src/
  ...
  components/  layout/ (TopBar, Footer) · catalog/ (CatalogPage, FilterBar, VizCard, CardPreview + previewFormat — S3‑th)
               viz/ (VizPage, AboutData, Pager — S3‑fx) · pages/ (AboutPage, NotFound) · AppStateProvider.tsx
...
**`RankedBar` baseline (S3‑fx):** bars start at 0 unless an entry passes `baseline` — only a scale whose floor is part
of its definition, taken from the entry's `data.ts` (GPI: `SCORE_MIN` = 1 on the 1–5 scale), never the data minimum
(the smallest bar would vanish). With a baseline the first tick is the baseline (`axisTicks`: ticks closer than 28 px
to it are dropped) and `axisLabel` draws one line above the ticks (+16 px axis height only when set). Without the
options the SVG is byte‑identical to the pre‑S3‑fx renderer (checked against it on 28 fixtures). The domain is still
per page: `[baseline, largest value on the page]`.
**Paging:** every paged ranking uses `components/viz/Pager.tsx` (the whole "Rows" field, ‹ select ›) with labels from
`pageRanges(total, size)`; the select is never narrower than its longest label (`--pager-ch` × 1ch + 3rem, tabular
digits) and the field never shrinks (`.controls .field-pager { flex: 0 0 auto }`) — on a narrow row it wraps.
...
- **Content‑sized flex fields (S3‑fx):** `.field select { width: 100% }` inside a field that sizes to its content
  (`flex: 0 0 auto`) resolves against the field's own intrinsic width — the select grew to the whole pager and pushed
  the › button out of the row (768 px). Reset `width: auto` there and set a `min-width` instead.
...
**S3‑fx (2026‑09‑25):** the two backlog items of 2026‑09‑25 are done — `RankedBar` optional baseline (GPI) and the
phone pager (shared `Pager`). Backlog: empty.
...
- **S3‑fx** (2026‑09‑25) — the two §13 backlog items (owner‑approved in S3‑rb). Audit on the built site (Playwright,
  Chromium): no shared pager — the same ‹ select › block and `pageOptions` were copied into 7 pages (gdp‑by‑country,
  gdp‑ppp‑per‑capita, population‑by‑country, land‑area, crime‑index, global‑peace‑index, births‑deaths‑per‑day);
  `.controls .field` (`flex: 1 1 10rem`) let the field shrink, the two buttons took 83 px and the select kept 44 px of
  text room for a 62–68 px label → clipped at 360 and 390 on all 7, at 768 on GPI and births‑deaths‑per‑day; no
  horizontal scroll anywhere. `renderRankedBar`: domain `[0, page max]`, so GPI page 1 (1.161–1.538) drew 75–100 %
  bars and a phone axis of just "0 · 1".
  Owner decisions (one question, all as recommended): (a) keep ‹ select ›, sized so it never clips, the field wraps
  instead of shrinking; (b) baseline on `global-peace-index` only — GDP, population, density, robots and Numbeo 0–100
  keep 0 (zero is part of those scales); (c) axis title + first tick = baseline.
  Built: `pageRanges(total, size)` in `lib/paginate.ts` (a last page of one row is "166", not "166–166"; an empty list
  is one page "0"); `components/viz/Pager.tsx` (the whole Rows field, `--pager-ch` = longest label) replaces the block
  in all 7 pages; CSS `.pager select { flex: 1 1 0; width: auto; min-width: calc(var(--pager-ch) * 1ch + 3rem);
  tabular-nums }` + `.controls .field-pager { flex: 0 0 auto }`. `renderRankedBar` options `baseline` (default 0) and
  `axisLabel`, pure `rankedBarDomain` + `axisTicks`, `.clamp(true)`; `RankedBar.tsx` passes them through; `.rb-axis-label`
  (`--tx2`, 12 px). GPI: `baseline={SCORE_MIN}`, title EN "Score on a 1–5 scale · bars start at 1" / UK "Бал за шкалою
  1–5 · стовпці від 1" (numbers from `SCORE_MIN`/`SCORE_MAX`), the same words in the chart's `aria-label`.
  A first build put the › button outside the row at 768 (the `width: 100%` of `.field select` inside a content‑sized
  field — §12); fixed before the final check.
  Tests: `test-ranked-bar.ts` 10 → 16 (no option = same SVG as baseline 0, domain/ticks from 0, no title; baseline
  → widths ∝ value − 1, IS/HU 0.30 vs 0.75 from 0; values at/below the baseline → 0 px, never negative; `axisTicks`;
  title above the ticks, +16 px, removed when unset, text not markup; phone layout at x = 0, ellipsis + `<title>` at
  160 px); new `test-pager.ts` (7: GPI ranges, one‑row last page, empty list, every range = `paginate` from/to for
  totals 1–300 × sizes 10/15/25, ≤ 7 characters on every real entry, CSS guard, every paged entry uses `<Pager>`);
  mutation check — dropping the baseline or restoring the shrinking field fails them. Smoke 982 → 999 (GPI pager
  markup, `--pager-ch:7`, disabled ‹ on page 1 and › on page 11, EN + UK wording, no pager in the table view; crime
  last option "166"). Old vs new renderer without options: 28 fixtures (4 row sets × 7 widths) byte‑identical.
  `verify` green in a scratch copy (27 test files · 999 smoke checks · build; GPI chunk 5.1 → 4.9 kB gzip, shared
  `Pager` chunk 0.76 kB gzip). Playwright on the built site, 7 pages × 360 / 390 / 768 / 1280: pager 212 px, select
  118 px (95 px text room for ≤ 68 px), no control outside its row, no horizontal scroll, 0 console errors; GPI EN/UK,
  dark/light, both orders, page 6 and the table view checked.
  Branch (proposed) `s3-fx-pager-baseline`. Left in the live folder: `dist-s3fx/` (gitignored builds for the
  screenshots) and `.git/index.lock.agent-stale-2026-09-25` — this session ran `git status` in the live folder
  against §12; the lock was renamed so git is not blocked — both safe to delete.
  The gallery card preview of GPI already used the 1–5 scale since S3‑th (`extremeRows(…, [SCORE_MIN, SCORE_MAX])`),
  so card and page now agree.
  Owner's Safari check (Responsive Design Mode, 2026‑09‑25): GPI at 360 (dark, light), 390 and 768 (light) — pager
  not clipped (Safari's ⌃⌄ arrow fits), axis title above the ticks, first tick 1 at the bars' start, no overflow.
  Open: Safari with the longest label (`?page=11`, "151–163") and `births-deaths-per-day` at 768 not shown yet;
  pre‑existing, not changed: a segmented group's label ("View") sits lower than the field labels in the same row
  (2.25 vs 2.6 rem controls).
