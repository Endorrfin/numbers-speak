...
/** timeline = continuous months, Jan 2022 .. Nov 2025 (default). seasonal = year-over-year, Jan-Dec. */
export const SHOWS = ['timeline', 'seasonal'] as const;
export type Show = (typeof SHOWS)[number];
export type View = 'chart' | 'table';

export type VgState = { show: Show; view: View };

const isShow = (v: string | undefined): v is Show => (SHOWS as readonly string[]).includes(v ?? '');

export function parseVgState(params: VizParams): VgState {
  return {
...
export function toVgParams(state: VgState): VizParams {
